...
void enableRawMode () {
    ...
    int numrows;
    erow row;
    // make sure that the terminal is reset to original values no matter where
    // exit is from
    ...
    // IXON makes sure that ctrl-s and ctrl-q don't stop the transmission of
    // data ICRNL disables ctrl-m and makes it the carriage return(\r) instead
    // of translating to newline(\n) BRKINT, INPCK, ISTRIP all legacy
    // traditions and likely don't do anything in modern applications
...
void editorFindCallback (char *query, int key) {
    ...
    for (i = 0; i < E.numrows; i++) {
        ...
        if (match) {
            ...
            editorSetStatusMessage ("Search term not found");
        }
    }
}
...
void editorRefreshScreen () {
    ...
    // [2J means clear the entire screen (1 would be clear up to the cursor and
    // 0 would be clear from the cursor to the end of the screen abAppend(&ab,
    // "\x1b[2J", 4); removed to just erase one line at a time in
    // editorDrawRows Moves the cursor to the top left
    abAppend (&ab, "\x1b[H", 3);
